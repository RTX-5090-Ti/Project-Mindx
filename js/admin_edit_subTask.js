...
let desSubTaskEdit = JSON.parse(localStorage.getItem("desSubTaskEdit")) || [];
...
listTaskToAdd.forEach((item) => {
  if (item.taskDes.trim() === desSubTaskEdit.desTask.trim()) {
    // console.log(item);
    item.subTask.forEach((icon) => {
      if (icon.subDes.trim() === desSubTaskEdit.desSubTask.trim()) {
        console.log(icon);
